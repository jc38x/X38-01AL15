...
% EDIT 04/08/2015 - urlwrite timeout + retries + apikey
...
keyindex = keyindex + 1;
maxkeys = maxkeys - 1;
if (keyindex <= size(keyring, 1) && maxkeys > 0)
    warning('Google Static Maps Api key change');
    attemptsleft = retries;
    continue;
end

error(sprintf(['Unable to download map form Google Servers.\n' ...
