...
% Plots a map on the current axes using a local Mapproxy installation
% This function is an edit of plot_google_map Version 1.5 retrieved from
% http://www.mathworks.com/matlabcentral/fileexchange/27627-zoharby-plot-google-map
% References
% http://wiki.openstreetmap.org/wiki/Slippy_map_tilenames
% http://mapproxy.org/docs/nightly/services.html#tiled-map-services-tms
% 
% Excerpt from plot_google_map:
%
...
mphost  = CONFIG.MAPPROXYHOST;
mpport  = CONFIG.MAPPROXYPORT;
mplayer = CONFIG.MAPPROXYLAYER;
mpproj  = CONFIG.MAPPROXYPROJ;
...
ax_position = get(axHandle,'position');        
set(axHandle,'Units',org_units)
width = ceil(ax_position(3));
height = ceil(ax_position(4));
...
% Tile stitching
% Central tile x,y
[xct, yct] = latlon2xy(lat, lon, zoomlevel);
n = 2 .^ zoomlevel;
ixct = fix(xct);
iyct = fix(yct);
fxct = abs(xct - ixct);
...
% Adjacent tiles x,y
inilx = floor(fxct * tileSize);
iniuy = floor(fyct * tileSize);
inirx = tileSize - inilx;
inidy = tileSize - iniuy;
wsx = floor(width / 2);
hsy = floor(height / 2);
displx = ceil((wsx - inilx) / tileSize);
dispuy = ceil((hsy - iniuy) / tileSize);
disprx = ceil((wsx - inirx) / tileSize);
dispdy = ceil((hsy - inidy) / tileSize);
rxmap = (ixct-displx):1:(ixct+disprx);
rymap = (iyct-dispuy):1:(iyct+dispdy);

% Coerce tiles to bounding box using wraparound [0, (2^zoomlevel)-1]
xmap = rxmap;
ymap = rymap;
c = n - 1;
xmap(xmap < 0) = xmap(xmap < 0) + ones(size(xmap(xmap < 0))) .* n;
ymap(ymap < 0) = ymap(ymap < 0) + ones(size(ymap(ymap < 0))) .* n;
xmap(xmap > c) = xmap(xmap > c) - ones(size(xmap(xmap > c))) .* n;
ymap(ymap > c) = ymap(ymap > c) - ones(size(ymap(ymap > c))) .* n;

% Map size in tiles
ylen = size(ymap, 2);
xlen = size(xmap, 2);

% Get tiles from server and build map image
filename = 'tmp.png';
preamble = ['http://' mphost ':' mpport '/tiles/' mplayer '/' mpproj];
zoomstr = num2str(zoomlevel);
baseurl = [preamble '/' zoomstr];
tilemap = [];
%convertNeeded = 0;
...
for yc = 1:ylen
    row = [];
    for xc = 1:xlen
        xstr = num2str(xmap(xc));
        ystr = num2str(ymap(yc));
        url = [baseurl  '/' xstr '/' ystr '.png?origin=nw'];

        while (true)
        [~, status] = urlwrite(url,filename, 'Timeout', timeout);
        if (status), break; end
        ...
        error(['Unable to download map from Mapproxy. ' ...
               'To debug, try pasting the following URL in ' ...
               'your browser, which may result in a more ' ...
               'informative error: ' url]);
        %varargout{1} = [];
        %varargout{2} = [];
        %varargout{3} = [];
        %return
...
dx = abs(rxmap(1) - ixct);
dy = abs(rymap(1) - iyct);
xcp = dx * tileSize + inilx + 1;
ycp = dy * tileSize + iniuy + 1;
ysl = (ycp - hsy):(ycp + hsy - 1);
xsl = (xcp - wsx):(xcp + wsx - 1);
tilemap = tilemap(ysl, xsl, :);

% Scale map image
if (scale ~= 1)
    tilemap = imresize(tilemap, 'Scale', scale, 'Method', scalingmode);
    tilemap(tilemap > 1) = 1;
    tilemap(tilemap < 0) = 0;
end

% Compatibility with image conversion code
M = tilemap;% .* 255;
...
function [x, y] = latlon2xy(lat, lon, zoomlevel)
n = 2 .^ zoomlevel;
x = n * ((lon + 180) / 360);
radlat = lat * pi / 180;
y = n * (1 - (log(tan(radlat) + sec(radlat)) / pi)) / 2;
%**************************************************************************